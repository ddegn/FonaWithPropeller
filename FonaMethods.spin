...
PRI InitFona
  ...
  'Header#TX_FROM_FONA = 23
  'Header#RING_INDICATOR = 24
  'Header#KEY = 25
  'Header#POWER_STATUS = 26
  'Header#RESET
PUB CheckNetwork | timer

  result := CheckOnState 
  ifnot result
    result := Header#FONA_OFF_NETWORK 
    return

  repeat while ina[Header#NETWORK_STATUS]

  timer := -cnt

  repeat until ina[Header#NETWORK_STATUS]

  timer += cnt

  timer /= clkfreq / 1000

  if timer < Header#NETWORK_GPRS_DATA_ACTIVE * 11 / 10
    result := Header#GPRS_DATA_ACTIVE_NETWORK
  elseif timer < Header#NETWORK_NO_CONNECTION * 11 / 10
    result := Header#NO_CONNECTION_NETWORK
  else
    result := Header#READY_NETWORK 
    
PUB CheckRing
'' This should be checked at least once every 120ms.

  result := ina[Header#RING_INDICATOR]
  if result
    result := 0
  else
    result := 1
...
PUB CheckOnState
  ...
  result := ina[Header#POWER_STATUS]

PUB SetPower(state) | attempts
